...
const parseMetaCSV = (text) => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const date = f[0], campaign = f[1];
    if (!date || !date.match(/^\d{4}-/)) continue;
    const spend = parseBR(f[2]), impressions = Math.round(parseBR(f[3])), clicks = Math.round(parseBR(f[4]));
    const contacts = Math.round(parseBR(f[6])), costPC = contacts > 0 ? parseBR(f[7]) : null;
...
const parseGoogleCSV = (text) => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const date = f[0], campaign = f[1];
    if (!date || !date.match(/^\d{4}-/)) continue;
    const spend = parseBR(f[3]), impressions = Math.round(parseBR(f[4])), clicks = Math.round(parseBR(f[5]));
    const contacts = Math.round(parseBR(f[7])), costPC = contacts > 0 ? parseBR(f[8]) : null;
...
const aggregateByDate = (rows) => {
  ...
  rows.forEach(r => {
    ...
    const d = byDate[r.date]; d.spend+=r.spend; d.impressions+=r.impressions; d.clicks+=r.clicks; d.contacts+=r.contacts;
  });
...
const computeKPIs = (rows) => {
  ...
  return { spend: Math.round(spend*100)/100, impressions, clicks, contacts,
    conv_rate: clicks>0 ? Math.round((contacts/clicks)*10000)/100 : 0,
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const all=[...parseMetaCSV(m),...parseGoogleCSV(g)];
      if(!all.length) throw new Error("Nenhum dado encontrado");
      setRawData(all); setLastUpdate(new Date()); setLoading(false);
  ...
  const compPeriod = useMemo(() => {
    if (compareMode === "none" || !dateFrom || !dateTo) return null;
    if (compareMode === "previous") {
      const days = daysBetween(dateFrom, dateTo);
      const cTo = addDays(dateFrom, -1);
      const cFrom = addDays(dateFrom, -(days + 1));
      return { from: cFrom, to: cTo, label: `${cFrom} a ${cTo}` };
    }
    if (compareMode === "yoy") {
      const cFrom = addDays(dateFrom, -365);
      const cTo = addDays(dateTo, -365);
      return { from: cFrom, to: cTo, label: `${cFrom} a ${cTo}` };
    }
    if (compareMode === "custom" && compFrom && compTo) {
      return { from: compFrom, to: compTo, label: `${compFrom} a ${compTo}` };
    }
    return null;
  }, [compareMode, dateFrom, dateTo, compFrom, compTo]);

  // Comparison data
  const compData = useMemo(() => {
    if (!compPeriod) return [];
    let rows = filtered.filter(r => r.date >= compPeriod.from && r.date <= compPeriod.to);
    return aggregateByDate(rows);
  }, [filtered, compPeriod]);
  ...
  // Merge chart data with comparison for overlay
  const chartMerged = useMemo(() => {
    if (!compPeriod || compData.length === 0) return chart30.map(r => ({...r, comp_spend:null, comp_contacts:null}));
    const compSorted = [...compData].sort((a,b)=>a.date.localeCompare(b.date)).slice(-30);
    return chart30.map((r, i) => {
      const comp = compSorted[i];
      const result = {...r};
      Object.keys(metricOptions).forEach(k => { result["comp_"+k] = comp ? (comp[k] ?? null) : null; });
      result.comp_label = comp ? comp.date : "";
      return result;
    });
  }, [chart30, compData, compPeriod]);
  ...
  const chipStyle=(active)=>({padding:"5px 14px",border:"1px solid "+(active?C.green:C.greenDark),borderRadius:20,cursor:"pointer",fontSize:11,fontWeight:500,fontFamily:"Inter",transition:"all .2s",background:active?C.green+"22":"transparent",color:active?C.green:C.grayLight});
  const toggleSort=(col)=>{if(sortCol===col)setSortDir(d=>d==="asc"?"desc":"asc");else{setSortCol(col);setSortDir("desc");}setPage(0);};
  const arrow=(col)=>sortCol===col?(sortDir==="asc"?" \u25B2":" \u25BC"):"";
  const btnStyle=(active)=>({padding:mob?"6px 12px":"8px 16px",border:"none",borderRadius:6,cursor:"pointer",fontFamily:"Poppins",fontSize:mob?10:12,fontWeight:600,transition:"all .2s",background:active?C.green:C.dark2,color:active?C.dark:C.grayLight,boxShadow:active?"0 0 12px rgba(58,232,96,.3)":"none"});
  const svcBtnStyle=(active)=>({padding:mob?"5px 8px":"6px 14px",border:active?"2px solid "+C.green:"1px solid "+C.greenDark,borderRadius:6,cursor:"pointer",fontFamily:"Inter",fontSize:mob?10:11,fontWeight:600,transition:"all .2s",background:active?C.green+"18":C.dark2,color:active?C.green:C.grayLight});
  const compBtnStyle=(active)=>({padding:mob?"5px 8px":"6px 12px",border:active?"2px solid "+C.yellow:"1px solid "+C.greenDark,borderRadius:6,cursor:"pointer",fontFamily:"Inter",fontSize:mob?9:10,fontWeight:600,transition:"all .2s",background:active?C.yellow+"15":C.dark2,color:active?C.yellow:C.gray});

  // Delta calculation
  const delta = (curr, prev, invert) => {
    if (prev === null || prev === undefined || prev === 0 || curr === null || curr === undefined) return null;
    const pct = ((curr - prev) / Math.abs(prev)) * 100;
    return { pct: Math.round(pct * 10) / 10, positive: invert ? pct < 0 : pct > 0 };
  };

  const DeltaBadge = ({curr, prev, invert}) => {
    const d = delta(curr, prev, invert);
    if (!d) return null;
    const color = d.positive ? C.green : C.red;
    const arrow = d.pct > 0 ? "\u25B2" : d.pct < 0 ? "\u25BC" : "";
    return <span style={{fontSize: mob?8:10, fontWeight:600, color, marginLeft:4}}>{arrow} {Math.abs(d.pct).toFixed(1)}%</span>;
  };
  ...
  if (error) return (
    <div style={{background:C.dark,minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"Inter",padding:32}}>
      <div style={{color:C.red,fontSize:18,fontWeight:700,fontFamily:"Poppins",marginBottom:8}}>Erro ao carregar dados</div>
  ...
  return (
    <div style={{background:C.dark,minHeight:"100vh",color:C.white,fontFamily:"Inter, sans-serif"}}>
      ...
      <div style={{background:C.dark2,padding:pad,display:"flex",flexDirection:mob?"column":"row",alignItems:mob?"flex-start":"center",justifyContent:"space-between",gap:mob?10:0,borderBottom:"2px solid "+C.greenDark}}>
        <div>
          <div style={{fontFamily:"Poppins",fontSize:mob?18:24,fontWeight:700,letterSpacing:-0.5}}>
      ...
      <div style={{padding:pad}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:mob?16:24}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",paddingTop:6,borderTop:"1px solid "+C.greenDark+"44"}}>
            ...
            <button style={compBtnStyle(compareMode==="previous")} onClick={()=>{if(!dateFrom||!dateTo){alert("Selecione um periodo primeiro");return;}setCompareMode("previous");}}>Periodo anterior</button>
            <button style={compBtnStyle(compareMode==="yoy")} onClick={()=>{if(!dateFrom||!dateTo){alert("Selecione um periodo primeiro");return;}setCompareMode("yoy");}}>Ano anterior</button>
            <button style={compBtnStyle(compareMode==="custom")} onClick={()=>setCompareMode("custom")}>Personalizado</button>
          </div>
          {compareMode==="custom"&&(
            <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:mob?"wrap":"nowrap",paddingLeft:mob?0:20}}>
          ...
          {comparing&&compPeriod&&(
            <div style={{fontSize:11,color:C.yellow,fontStyle:"italic"}}>
              Comparando com: {compPeriod.label} ({compData.length} dias)
            </div>
          )}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:mob?"repeat(2, 1fr)":compact?"repeat(3, 1fr)":"repeat(6, 1fr)",gap:mob?8:compact?12:16,marginBottom:mob?16:28}}>
          {[
            {label:"Valor Usado",value:fmtBRL(kpis.spend),sub:"total investido",key:"spend",inv:true},
            {label:"Impressoes",value:fmt(kpis.impressions),sub:"alcance",key:"impressions"},
            {label:"Cliques",value:fmt(kpis.clicks),sub:"todos",key:"clicks"},
            {label:"Tx Conv.",value:fmtPct(kpis.conv_rate),sub:"cliques > contatos",key:"conv_rate"},
            {label:"Contatos",value:kpis.contacts.toLocaleString("pt-BR"),sub:"por mensagem",key:"contacts"},
            {label:"Custo/Cont.",value:fmtBRL(kpis.cost_per_contact),sub:"medio",key:"cost_per_contact",inv:true}
          ].map((k,i)=>(
            <div key={i} style={{background:C.dark2,borderRadius:10,padding:mob?"10px":"20px 16px",position:"relative",overflow:"hidden",minWidth:0}}>
              ...
              {comparing&&compKpis&&(
                <div style={{fontSize:mob?8:9,color:C.yellow,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                  Ant: {k.key==="spend"||k.key==="cost_per_contact"?fmtBRL(compKpis[k.key]):k.key==="conv_rate"?fmtPct(compKpis[k.key]):k.key==="contacts"?compKpis[k.key]?.toLocaleString("pt-BR"):fmt(compKpis[k.key])}
                </div>
              )}
            ...
            <ResponsiveContainer width="100%" height={mob?160:200}>
              <BarChart data={chartMerged}>
                ...
                <Tooltip contentStyle={{background:C.dark,border:"1px solid "+C.greenDark,borderRadius:8,color:C.white,fontSize:11}}
                  formatter={(v,name)=>[v!=null?"R$ "+v.toFixed(2):"\u2014",name==="comp_spend"?"Periodo anterior":"Atual"]}/>
                <Bar dataKey="spend" fill={C.green} radius={[3,3,0,0]} name="Atual"/>
                {comparing&&<Bar dataKey="comp_spend" fill={C.yellow+"88"} radius={[3,3,0,0]} name="Anterior"/>}
              </BarChart>
            </ResponsiveContainer>
            {comparing&&<div style={{display:"flex",gap:16,justifyContent:"center",marginTop:8}}>
              <span style={{fontSize:10,color:C.green}}>● Atual</span>
              <span style={{fontSize:10,color:C.yellow}}>● Anterior</span>
            </div>}
          </div>
          <div style={{background:C.dark2,borderRadius:10,padding:mob?"14px":"20px"}}>
            ...
            <ResponsiveContainer width="100%" height={mob?160:200}>
              <LineChart data={chartMerged}>
                ...
                <YAxis tick={{fill:C.gray,fontSize:mob?7:9}} width={mob?35:60}/>
                <Tooltip contentStyle={{background:C.dark,border:"1px solid "+C.greenDark,borderRadius:8,color:C.white,fontSize:11}}
                  formatter={(v,name)=>[v!=null?metricOptions[rightMetric].fmtTip(v):"\u2014",name.startsWith("comp_")?"Anterior":"Atual"]}/>
                <Line type="monotone" dataKey={rightMetric} stroke={C.green} strokeWidth={2} dot={{r:2,fill:C.green}} name="Atual"/>
                {comparing&&<Line type="monotone" dataKey={"comp_"+rightMetric} stroke={C.yellow} strokeWidth={2} strokeDasharray="5 5" dot={{r:2,fill:C.yellow}} name="Anterior"/>}
              </LineChart>
            </ResponsiveContainer>
            {comparing&&<div style={{display:"flex",gap:16,justifyContent:"center",marginTop:8}}>
              <span style={{fontSize:10,color:C.green}}>\u2014 Atual</span>
